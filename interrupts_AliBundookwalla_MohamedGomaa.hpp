...
struct PCB{
    ...
    int             start_time;        // When process started running
    unsigned int    processing_time;   // Total CPU time needed
    unsigned int    remaining_time;    // Remaining CPU time
    int             partition_number;
    enum states     state;
    unsigned int    io_freq;
    unsigned int    io_duration;
    unsigned int    time_slice;        // For Round Robin time tracking
    unsigned int    last_cpu_burst;    // Last time I/O was triggered
    unsigned int remaining_io_time;  // Track current I/O remaining time
};
